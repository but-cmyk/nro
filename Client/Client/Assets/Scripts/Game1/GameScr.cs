namespace Game1
{
    ...
    public class GameScr : mScreen, IChatable
    {
    	...
    	public static void initVersionFromRMS()
    	{
    	...
    	public void readEfect()
    	{
    		...
    		try
    		{
    			sbyte[] data = Rms.loadRMS("NR_effect");
    			if (data == null)
    			{
    				return;
    			}
    		...
    		{
    			Cout.LogError("Loi readEfect: " + ex.ToString());
    		}
    		finally
    		{
    			try
    	...
    	public static EffectCharPaint getEffect(int id)
    	{
    		try
    		{
    			if (efs == null)
    			...
    			if (efs != null && id - 1 >= 0 && id - 1 < efs.Length)
    			{
    				return efs[id - 1];
    			}
    		}
    	...
    	public bool isMeCanAttackMob(Mob m)
    	{
    		...
    		if (m.isMobMe)
    		{
    			...
    			if (@char.cTypePk == 5 || Char.myCharz().cTypePk == 5)
    			{
    				return true;
    			}
    	...
    	private void checkAuto()
    	{
    		...
    		if (GameCanvas.gameTick % 5 == 0 && auto > 0 && Char.myCharz().currentMovePoint == null)
    		{
    			...
    			{
    				if (Char.myCharz().myskill != null && Char.myCharz().myskill.paintCanNotUseSkill)
    				{
    					return;
    				}
    				doFire(false, true);
    		...
    		else
    		{
    			...
    			else if (isAttack())
    			{
    				...
    				else
    				{
    					...
    					{
    						skillPaint = sks[Char.myCharz().myskill.skillId];
    					}
    	...
    	{
    		int num = (c.cHPFull > 0) ? (int)(c.cHP * hpBarW / c.cHPFull) : 0;
    		int num2 = c.cMP * mpBarW;
    		int num3 = (c.cHPFull > 0) ? (int)(dHP * hpBarW / c.cHPFull) : 0;
    		int num4 = dMP * mpBarW;
    	...
    	private void paintImageBar(mGraphics g, bool isLeft, Char c)
    	{
    		if (c != null)
    		{
    			...
    			{
    				num = (c.cHPFull > 0) ? (int)(dHP * hpBarW / c.cHPFull) : 0;
    				num2 = (c.cMPFull > 0) ? dMP * mpBarW / c.cMPFull : 0;
    				num3 = (c.cHPFull > 0) ? (int)(c.cHP * hpBarW / c.cHPFull) : 0;
    				num4 = (c.cMPFull > 0) ? c.cMP * mpBarW / c.cMPFull : 0;
    			}
    			else
    			{
    				num = (c.cHPFull > 0) ? (int)(c.dHP * hpBarW / c.cHPFull) : 0;
    			...
    			{
    				int w = (Char.myCharz().maxPowerPoint > 0) ? (Char.myCharz().powerPoint * spBarW / Char.myCharz().maxPowerPoint) : 0;
    	...
    	{
    		if (Char.myCharz() == null || Char.myCharz().taskMaint == null || GameCanvas.panel == null || GameCanvas.menu == null || ChatTextField.gI() == null)
    		{
    			return;
    		}
    		if (mobCapcha != null)
    	...
    	{
    		if (splashState == null || imgSplash == null || splashF == null || splashX == null || splashY == null || splashDir == null)
    		{
    			return;
    		}
    		for (int i = 0; i < 2; i++)
    	...
    	public static void paintHPBar_NEW(mGraphics g, int x, int y, Char c)
    	{
    		...
    		num2 = (c.cHPFull > 0) ? (c.cHP * width / c.cHPFull) : 0;
    		if (num2 <= 0)
    		{
    			num2 = 1;
    		}
    		...
    		num3 = (c.cMPFull > 0) ? (c.cMP * width / c.cMPFull) : 0;
    		if (num3 <= 0)
    		{
    			num3 = 1;
    		}
