    ...
    public class EffectPanel : Effect2
    {
    	...
    	public static void addServerEffect(int id, int cx, int cy, int loopCount)
    	{
            try
            {
                ...
                if (GameScr.efs == null || id - 1 < 0 || id - 1 >= GameScr.efs.Length) return;
                EffectCharPaint eff = GameScr.efs[id - 1];
                ...
                if (Effect2.vEffect3 != null) Effect2.vEffect3.addElement(effectPanel);
            }
            catch (Exception) { }
    	}
