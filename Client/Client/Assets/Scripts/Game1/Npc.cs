namespace Game1
{
    public class Npc : Char
    {
		...
		public static void EnsureNpcTemplateCapacity(int requiredId)
		{
			if (requiredId < 0) return;
			if (arrNpcTemplate == null)
			{
				arrNpcTemplate = new NpcTemplate[System.Math.Max(256, requiredId + 1)];
			}
			else if (requiredId >= arrNpcTemplate.Length)
			{
				System.Array.Resize(ref arrNpcTemplate, System.Math.Max(arrNpcTemplate.Length * 2, requiredId + 1));
			}
		}
    	...
    	public Npc(int npcId, int status, int cx, int cy, int templateId, int avatar)
    	{
    		...
    		if (npcId != -1)
    		{
    			EnsureNpcTemplateCapacity(templateId);
    			if (arrNpcTemplate != null && templateId >= 0 && templateId < arrNpcTemplate.Length)
    			{
    			...
    			if (template == null)
    			{
    				...
    				template.npcTemplateId = (sbyte)templateId;
    				template.name = string.Empty;
    				if (arrNpcTemplate != null && templateId >= 0 && templateId < arrNpcTemplate.Length)
    				{
    					arrNpcTemplate[templateId] = template;
    				}
    			}
    		}
