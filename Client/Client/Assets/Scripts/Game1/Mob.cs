namespace Game1
{
    ...
    public class Mob : IMapObject
    {
    	...
    	public bool isBigBoss()
    	{
    		...
    		public static void EnsureMobTemplateCapacity(int requiredId)
		{
			if (requiredId < 0) return;
			if (arrMobTemplate == null)
			{
				arrMobTemplate = new MobTemplate[System.Math.Max(256, requiredId + 1)];
			}
			else if (requiredId >= arrMobTemplate.Length)
			{
				System.Array.Resize(ref arrMobTemplate, System.Math.Max(arrMobTemplate.Length * 2, requiredId + 1));
			}
		}
		...
		public void getData()
		{
			...
			if (arrMobTemplate == null || templateId < 0 || templateId >= arrMobTemplate.Length)
			{
				return;
			}
    	...
    	{
    		if (arrMobTemplate == null) return;
    		int num = 0;
    		for (int i = 0; i < arrMobTemplate.Length; i++)
    		{
    			if (arrMobTemplate[i] != null && arrMobTemplate[i].data != null)
    			{
    				num++;
    			}
    		}
    	...
    	public bool isPaint()
    	{
    		...
    		MobTemplate t = getTemplate();
    		if (t == null || t.data == null || t.data.img == null || status == 0)
    		{
    			return false;
    		}
    		return true;
    	...
    	{
    		MobTemplate t = getTemplate();
    		if (t == null || t.data == null || status == 0)
    		{
    			return false;
    		}
    		return true;
    	...
    	public virtual void paint(mGraphics g)
    	{
    		...
    		if (isMafuba)
    		{
    			if (!changBody)
    			{
    				if (arrMobTemplate != null && templateId >= 0 && templateId < arrMobTemplate.Length && arrMobTemplate[templateId] != null && arrMobTemplate[templateId].data != null)
    				{
    	...
    	{
    		MobTemplate t = getTemplate();
    		if (isGetFr && isTypeNewMod() && t != null && t.data != null)
    		{
    	...
    	{
    		MobTemplate t = getTemplate();
    		if (t != null && t.data != null && t.data.typeData == 2)
    		{
    			return true;
    		}
    		return false;
