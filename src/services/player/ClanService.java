...
public class ClanService {
        ...
        try {
            ...
            switch (action) {
                ...
                case ACCEPT_CREATE_CLAN:
                    if (TimeUtil.diffDate(new Date(), new Date(player.lastTimeRemoveClan), TimeUtil.DAY) < 1) {
                        long timeRemaining = player.lastTimeRemoveClan + (1000 * 60 * 60 * 24) - System.currentTimeMillis();
                        if (timeRemaining > 0) {
                            String timeFormatted = Util.msToTime(timeRemaining);
                            Service.gI().sendThongBao(player,
                                    "Vui lòng quay lại sau " + timeFormatted + " để thành lập bang hội khác.");
                        } else {
                            Service.gI().sendThongBao(player, "Bạn có thể tạo bang hội mới ngay.");
                        }
                        return;
                    }
    ...
    private void acceptJoinClan(Player player, int clanId) {
        try {
            ...
            if (diff > 0) {
                String timeLeft = Util.msToTime(diff);
                Service.gI().sendThongBao(player, "Sau khi rời bang, bạn cần chờ " + timeLeft + " trước khi có thể gia nhập lại.");
            } else {
                Service.gI().sendThongBao(player, "Bạn có thể gia nhập lại bang hội.");
            }
                ...
                if (flagBag.gold > 0) {
                    if (player.inventory.gold >= flagBag.gold) {
                        ...
                        Service.gI().sendThongBao(player, "Bạn không đủ vàng, còn thiếu "
                                + Util.powerToString(flagBag.gold - player.inventory.gold) + " vàng");
                        return;
                    }
                }
                if (flagBag.gem > 0) {
            ...
            if (clan != null) {
                ...
                try {
                    ...
                    for (ClanMember cm : clan.getMembers()) {
                        ...
                        msg.writer().writeUTF(Util.powerToString(cm.powerPoint));
