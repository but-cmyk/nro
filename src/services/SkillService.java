...
public class SkillService {
    ...
    public void updateSkillSpecial(Player player) {
        try {
            ...
            Logger.error("Error updateSkillSpecial");
        }
    }
    ...
    public void newSkillNotFocus(Player player, int status) {
        ...
        try {
            ...
            if (status == 20) {
                ...
                short pointY = newSkill._yPlayer;
            ...
            Logger.error("Error newSkillNotFocus");
        } finally {
            if (msg != null) {
                msg.cleanup();
            }
    ...
    public void learSkillSpecial(Player player, byte skillID) {
        ...
        try {
            ...
            assert curSkill != null;
            message.writer().writeShort(curSkill.skillId);
            player.sendMessage(message);
            message.cleanup();
        } catch (Exception e) {
            Logger.error("Error learSkillSpecial");
        } finally {
            if (message != null) {
                message.cleanup();
            }
    ...
    public void useSkillAttack(Player player, Player plTarget, Mob mobTarget) {
        ...
        switch (player.playerSkill.skillSelect.template.id) {
            case Skill.KAIOKEN: //kaioken
                long hpUse = Util.maxIntValue((double) player.nPoint.hpMax / 100 * 3);
                if (player.setClothes.thanVuTruKaio == 4) {
                    hpUse = Util.maxIntValue((double) player.nPoint.hpMax / 100 * 2);
                } else if (player.setClothes.thanVuTruKaio == 5) {
                    hpUse = Util.maxIntValue((double) player.nPoint.hpMax / 100 * 1);
                }
    ...
    private void playerAttackPlayer(Player plAtt, Player plInjure, boolean miss) {
        ...
        // 2. LOGIC PHẢN SÁT THƯƠNG (Ưu tiên cao nhất - Tính dựa trên dame GỐC)
        // Biến đánh dấu đã kích hoạt PST chưa để tránh tính trùng lặp
        boolean isPSTActive = false;

        if (!miss && plInjure.nPoint.tlPST > 0) {
            // A. Phản sát thương lên người đánh (plAtt) - Dựa trên dame GỐC
            // Hàm này gửi Message 56 -> Hiện dame đỏ trên đầu người đánh
            phanSatThuong(plAtt, plInjure, dameAttack);
            isPSTActive = true;

            // B. KHÔNG hủy sát thương - để logic Vô Hiệu Chưởng xử lý sau
            // Người bị đánh có thể vẫn nhận dame hoặc né tùy vào Vô Hiệu Chưởng
        }

        // 3. Logic Vô Hiệu Chưởng (Chạy sau PST, KHÔNG kiểm tra isPSTActive)
        if (!miss && plAtt.playerSkill.skillSelect != null) {
            ...
            if (skillId == Skill.KAMEJOKO || skillId == Skill.ANTOMIC || skillId == Skill.MASENKO) {
                if (plInjure.nPoint.voHieuChuong > 0 && Util.isTrue(plInjure.nPoint.voHieuChuong, 100)) {
                    miss = true;
                    dameAttack = 0;
                    // Lúc này PST đã được tính ở bước 2 dựa trên dame gốc
                    // Người đánh vẫn bị phản dame, nhưng người bị đánh không nhận dame
                }
            }
        }

        // 4. Trừ máu (Nếu dính Vô Hiệu thì dameAttack = 0)
        int dameHit = plInjure.injured(plAtt, miss ? 0 : dameAttack, false, false);

        // 5. Notify Server (Log chat)
        if (plAtt.playerSkill != null) {
            Skill skillSelect = plAtt.playerSkill.skillSelect;
            if (plAtt.isPl() && dameHit >= 1000000000) {
                ServerNotify.gI().notify(plAtt.name + ": đã đánh 1 chiêu " + skillSelect.template.name + " với sát thương là " + Util.formatNumber(dameHit));
            }
        }

        // 6. Logic PST bổ sung (Khiên năng lượng hoặc các hiệu ứng khác)
        // KHÔNG chạy nếu PST chính đã kích hoạt để tránh tính 2 lần
        if (!isPSTActive) {
        ...
        try {
            ...
            msg.writer().writeByte(1);
            msg.writer().writeInt((int) plInjure.id);     // ID người bị đánh (Target)
            msg.writer().writeByte(1);
            msg.writer().writeByte(0);

            // Nếu Vô Hiệu Chưởng kích hoạt: dameHit = 0 → Client hiện "Hụt" trên plInjure
            // Nhưng đã gọi phanSatThuong() ở bước 2 → Dame đỏ hiện trên plAtt
...
//    }
    ...
    switch (skillId) {
        case Skill.TRI_THUONG:
            if (intrinsic.id == 10) subTimeParam = intrinsic.param1;
            break;
        case Skill.THAI_DUONG_HA_SAN:
            if (intrinsic.id == 3) subTimeParam = intrinsic.param1;
            break;
        case Skill.QUA_CAU_KENH_KHI:
            if (intrinsic.id == 4) subTimeParam = intrinsic.param1;
            break;
        case Skill.KHIEN_NANG_LUONG:
            if (intrinsic.id == 5 || intrinsic.id == 15 || intrinsic.id == 20) subTimeParam = intrinsic.param1;
            break;
        case Skill.MAKANKOSAPPO:
            if (intrinsic.id == 11) subTimeParam = intrinsic.param1;
            break;
        case Skill.DE_TRUNG:
            if (intrinsic.id == 12) subTimeParam = intrinsic.param1;
            break;
        case Skill.TU_SAT:
            if (intrinsic.id == 19) subTimeParam = intrinsic.param1;
            break;
        case Skill.HUYT_SAO:
            if (intrinsic.id == 21) subTimeParam = intrinsic.param1;
            break;
        case Skill.MASENKO:
            if (intrinsic.id == 9) subTimeParam = intrinsic.param1;
            if (player.setClothes.nail == 4) subTimeParam += 20;
            if (player.setClothes.nail == 5) subTimeParam += 50;
            break;
    }

    int originalCoolDown = player.playerSkill.skillSelect.coolDown;

    // Nếu không có nội tại, chỉ cần đặt thời gian hồi chiêu gốc và thoát
    if (subTimeParam <= 0) {
        player.playerSkill.skillSelect.lastTimeUseThisSkill = System.currentTimeMillis();
        return;
    }

    // --- LOGIC CHO CÓ NỘI TẠI ---

    // 1. Tính toán thời gian
    long reducedTime = (long) originalCoolDown * subTimeParam / 100;
    long newCoolDown = originalCoolDown - reducedTime;
    if (newCoolDown < 0) newCoolDown = 0;

    // 2. Cập nhật trạng thái cho SERVER (Fix lỗi offline)
    // Đặt thời gian sử dụng vào quá khứ để server biết kỹ năng hồi sớm hơn.
    player.playerSkill.skillSelect.lastTimeUseThisSkill = System.currentTimeMillis() - reducedTime;

    // 3. Kích hoạt bộ đếm giờ cho CLIENT (Fix lỗi online)
    EffectSkillService.gI().setIntrinsic(player, skillId, (int) newCoolDown, System.currentTimeMillis());
}
