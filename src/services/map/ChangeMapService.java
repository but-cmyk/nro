...
//                // Chặn vào map nếu sức mạnh quá 1.500.000
    ...
    public void changeMapWaypoint(Player player) {
        ...
        if (player.zone != null && (player.zone.map.mapId == 45 || player.zone.map.mapId == 46)) {
...
//                if ((b.id == BossID.SO_4 || b.id == BossID.SO_3 || b.id == BossID.SO_2 || b.id == BossID.SO_1 || b.id == BossID.TIEU_DOI_TRUONG)
//                        && (TaskService.gI().getIdTask(player) < ConstTask.TASK_20_1 || TaskService.gI().getIdTask(player) > ConstTask.TASK_20_5)
//                        || (b.id == BossID.FIDE)
//                        && (TaskService.gI().getIdTask(player) < ConstTask.TASK_21_0 || TaskService.gI().getIdTask(player) > ConstTask.TASK_21_2)
//                        || (b.id == BossID.PIC || b.id == BossID.POC || b.id == BossID.KING_KONG)
//                        && (TaskService.gI().getIdTask(player) < ConstTask.TASK_25_1 || TaskService.gI().getIdTask(player) > ConstTask.TASK_25_3)
//                        || (b.id == BossID.XEN_BO_HUNG)
//                        && (TaskService.gI().getIdTask(player) < ConstTask.TASK_26_1 || TaskService.gI().getIdTask(player) > ConstTask.TASK_26_3)) {
//                    Service.gI().sendThongBao(player, "Đang trong giờ hỗ trợ nhiệm vụ , không thể vào khu " + zoneJoin.zoneId);
//                    int zone = 0;
//                    while (!zoneJoin.map.zones.get(zone).getBosses().isEmpty()) {
//                        zone++;
//                        if (zoneJoin.map.zones.get(zone).getBosses().isEmpty()) {
//                            return zoneJoin.map.zones.get(zone);
//                        }
//                    }
//                    return null;
//                }
