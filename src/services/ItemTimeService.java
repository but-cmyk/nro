...
import models.phoban.SnakeWay;
import models.phoban.DestronGas;
import models.phoban.RedRibbonHQ;
...
public class ItemTimeService {
    ...
    public void sendAllItemTime(Player player) {
        ...
        if (player.itemTime.isBuaTNSM) {
            sendItemTime(player, 11474, (int) ((TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeBuaTNSM)) / 1000));
        }
        ...
        if (player.itemTime.isUseCo4La) {
            sendItemTime(player, 13618, (int) ((TIME_CO - (System.currentTimeMillis() - player.itemTime.lastTimeUseCo4La)) / 1000));
        }
        if (player.itemTime.banhchung) {
            sendItemTime(player, 7080, (int) ((DEVNDUNG - (System.currentTimeMillis() - player.itemTime.banhchunglastTime)) / 1000));
        }
        if (player.itemTime.banhtet) {
            sendItemTime(player, 7079, (int) ((DEVNDUNG - (System.currentTimeMillis() - player.itemTime.banhtetlastTime)) / 1000));
        }
        if (player.itemTime.nguqua) {
            sendItemTime(player, 10889, (int) ((DEVNDUNG - (System.currentTimeMillis() - player.itemTime.nguqualastTime)) / 1000));
        }
