...
//    }
    ...
    public void setIsCaRot(Player player, int outFit, int time) {
        if (player.effectSkill != null) {
            player.effectSkill.isCaRot = true;
            player.effectSkill.idOutfitCaRot = outFit != -1 ? outFit : Util.nextInt(5);
            player.effectSkill.timeCaRot = time;
            player.effectSkill.lastTimeCaRot = System.currentTimeMillis();
            ItemTimeService.gI().sendItemTime(player, 4075, time / 1000);
            Service.gI().Send_Caitrang(player);
        }
    }
    
     public void setIsSocola2(Player player, int time) {
        if (MapService.gI().isMapMaBu(player.zone.map.mapId)) {
            player.nPoint.hp /= 2;
            PlayerService.gI().sendInfoHp(player);
            Service.gI().Send_Info_NV(player);
        }
        player.effectSkill.isSocola2 = true;
        player.effectSkill.timeSocola2 = time;
        player.effectSkill.lastTimeSocola2 = System.currentTimeMillis();
        ItemTimeService.gI().sendItemTime(player, 4132, time / 1000);
        Service.gI().Send_Caitrang(player);
       
    }

    public void removeSocola2(Player player) {
        player.effectSkill.isSocola2 = false;
        Service.gI().Send_Caitrang(player);
...
//    }
    ...
    public void sendEffectMonkey(Player player) {
        try {
            ...
            if (skill == null) {
                utils.Logger.log("Skill 'BIEN_KHI' không tồn tại cho người chơi: " + player.id);
                return;
            }
