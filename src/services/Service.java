...
public class Service {
    ...
    public void switchToRegisterScr(ISession session) {
        ...
        try {
            ...
            Logger.error("Error switchToRegisterScr");
        }
    }
    ...
    public void sendPetFollow(Player player, short smallId) {
        ...
        try {
            ...
            Logger.error("Error sendPetFollow");
        }
    }
    ...
    public List<String> ListCaption(int gender) {
        ...
        Captions.add("Ngọc Rồng Online Siêu Cấp");
        return Captions;
    }
    ...
    public void sendFlagBag(Player pl) {
        ...
        try {
            ...
            Logger.error("Error sendFlagBag");
        }
    }
    ...
    public void attackPlayer(Player pl, int idPlAnPem) {
        if (pl == null || pl.zone == null) {
        //    System.out.println("Người chơi hoặc khu vực không hợp lệ.");
            return;
        }
        Player player;
        ...
        if (player == null) {
       //     System.out.println("Không tìm thấy người chơi với ID: " + idPlAnPem);
            return;
        }
    ...
    public void sendTimeSkill(Player pl) {
        ...
        try {
            ...
            Logger.error("Error sendTimeSkill");
        }
    }
   ...
   public void showListTop(Player player, List<TOP> tops) {
        ...
        try {
            if (tops == null || tops.isEmpty()) {
                Service.gI().sendThongBao(player, "Chưa có dữ liệu bảng xếp hạng.");
                return;
            }
            ...
            for (int i = 0; i < tops.size(); i++) {
                ...
                // --- SỬA ĐOẠN NÀY ---
                // Bước 1: Ưu tiên lấy người chơi đang Online để có Avatar chính xác nhất
                if (top.getId_player() != -1) {
                    pl = Client.gI().getPlayer(top.getId_player());
                }

                // Bước 2: Nếu người chơi không Online (null), thì mới load từ Database
                if (pl == null && top.getId_player() != -1) {
                     pl = NDVSqlFetcher.loadById(top.getId_player());
                }
                // --------------------
                ...
                if (pl != null) {
                    ...
                    // Trường hợp không tìm thấy (Acc xóa, hoặc Top ảo)
                    msg.writer().writeInt(-1); 
                    msg.writer().writeShort(95); // Head mặc định
                    if (player.getSession().version > 214) {
                        msg.writer().writeShort(-1);
                    }
                    msg.writer().writeShort(96); // Body mặc định
                    msg.writer().writeShort(97); // Leg mặc định
                    msg.writer().writeUTF("---"); 
                }
