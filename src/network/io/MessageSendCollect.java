...
public class MessageSendCollect implements IMessageSendCollect {
    ...
    public Message readMessage(ISession session, DataInputStream dis) throws Exception {
        ...
        // --- BẮT ĐẦU ĐOẠN SỬA ---
        // Thêm kiểm tra kích thước tối đa (Ví dụ 200KB).
        // Nếu gói tin lớn hơn mức này -> Ngắt kết nối ngay.
        if (size > 200000 || size < 0) {
            throw new IOException("Goi tin qua lon hoac khong hop le: " + size);
        }
        // --- KẾT THÚC ĐOẠN SỬA ---
