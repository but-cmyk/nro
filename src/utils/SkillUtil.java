...
public class SkillUtil {
    ...
    public static boolean upSkillPet(List<Skill> skills, int index) {
        ...
        Skill currentSkill = skills.get(index);
        int tempId = currentSkill.template.id;
        int newLevel = currentSkill.point + 1;

        SkillTemplate template = findSkillTemplate(tempId);
        if (template == null || template.skillss == null) {
            return false;
        }

        // Nếu vượt quá số level của skill thì không cho up
        if (newLevel < 1 || newLevel > template.skillss.size()) {
            return false;
        }

        // Tạo skill mới từ template
        Skill upgraded = template.skillss.get(newLevel - 1);
        if (upgraded == null) {
            return false;
        }

        Skill newSkill = new Skill(upgraded);

        // Nếu có logic riêng cho slot thứ 2 thì nên để bằng hằng số / config
        if (index == 1) {
