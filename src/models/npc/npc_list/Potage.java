                ...
                if (player.idMark.isBaseMenu()) {
                    ...
                    if (BossClone == null) {
                        switch (select) {
                            ...
                            case 1 -> {
                                // Kiểm tra thời gian: Nếu chưa qua ngày mới VÀ không phải admin -> Chặn
                                if (!Util.isAfterMidnight(player.lastPkCommesonTime) && !player.isAdmin()) {
                                    Service.gI().sendThongBao(player, "Mỗi ngày chỉ được khiêu chiến 1 lần, hãy quay lại vào ngày mai!");
                                } else {
                                    // Gọi boss (Logic gọi new NhanBan thường nằm trong Service này)
