...
public class CADICH extends Boss {
    ...
    public CADICH(Zone zone, Clan clan,int level, int dame, int hp) throws Exception {
    ...
    public void reward(Player plKill) {
        int dropX = this.location.x - 15;
        int dropY = this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24);

        // [LOGIC MỚI] Rơi item dựa trên số player và level
        dropItemsByPlayers(plKill, dropX, dropY);

        // [LOGIC MỚI] Rơi item 459 từ level 50+
        if (levell >= 50) {
            dropRareItems(plKill, dropX, dropY);
        }
    }

    // [MỚI] Rơi item 19, 20 theo số lượng player
    private void dropItemsByPlayers(Player plKill, int dropX, int dropY) {
        int numPlayers = this.zone.getNumOfPlayers(); // Số player trong map
        if (numPlayers <= 0) numPlayers = 1; // Tối thiểu 1

        // Rơi Item 20 (1 túi cho mỗi player)
        for (int i = 0; i < numPlayers; i++) {
            int offsetX = dropX + (i * -15);
            ItemMap item20 = new ItemMap(this.zone, 20, 1, offsetX, dropY, (int) plKill.id);
            Service.getInstance().dropItemMap(this.zone, item20);
        }

        // Rơi Item 19 (1 túi cho mỗi 2 player)
        int numItem19 = Math.max(1, numPlayers / 2);
        for (int i = 0; i < numItem19; i++) {
            int offsetX = dropX + ((numPlayers + i) * -15);
            ItemMap item19 = new ItemMap(this.zone, 19, 1, offsetX, dropY, (int) plKill.id);
            Service.getInstance().dropItemMap(this.zone, item19);
        }
    }

    // [MỚI] Rơi item 459 từ level 50+
    private void dropRareItems(Player plKill, int dropX, int dropY) {
        int numItem459;
        int quantityPer459;

        if (levell >= 110) {
            // Level 110: Rơi 8 túi, mỗi túi 1 viên
            numItem459 = 8;
            quantityPer459 = 1;
        } else if (levell >= 100) {
            // Level 100-109: Rơi 6 túi, mỗi túi 1 viên
            numItem459 = 6;
            quantityPer459 = 1;
        } else if (levell >= 80) {
            // Level 80-99: Rơi 4 túi, mỗi túi 1 viên
            numItem459 = 4;
            quantityPer459 = 1;
        } else if (levell >= 60) {
            // Level 60-79: Rơi 3 túi, mỗi túi 1 viên
            numItem459 = 3;
            quantityPer459 = 1;
        } else {
            // Level 50-59: Rơi 2 túi, mỗi túi 1 viên
            numItem459 = 2;
            quantityPer459 = 1;
        }

        // Drop Item 459 với option random
        for (int i = 0; i < numItem459; i++) {
            int offsetX = dropX + (i * -20) - 100; // Cách xa item 19,20
            ItemMap item459 = new ItemMap(this.zone, 459, quantityPer459, offsetX, dropY, (int) plKill.id);
            item459.options.add(new Item.ItemOption(20,  90));
            Service.getInstance().dropItemMap(this.zone, item459);
        }
    }
