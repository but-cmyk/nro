...
public class PVPService {
    ...
    private static final int[] GOLD_CHALLENGE = {1000, 10000, 100000};
    ...
    public PVPService() {
        ...
        for (int i = 0; i < GOLD_CHALLENGE.length; i++) {
            this.optionsGoldChallenge[i] = Util.powerToString(GOLD_CHALLENGE[i]) + " vàng";
        }
    }
                            ...
                            if (Client.gI().getPlayer(playerId) == null) {
                                if (plMap != null && plMap.isBoss) {
                                    NpcService.gI().createMenuConMeo(player, ConstNpc.IGNORE_MENU,
                                            -1, plMap.name + " (sức mạnh " + Util.powerToString(plMap.nPoint.power)
    ...
    public void sendInvitePVP(Player pl, byte selectGold) {
        ...
        try {
            ...
            msg.writer().writeUTF(pl.name + " (sức mạnh " + Util.powerToString(pl.nPoint.getFullTN()) + ") muốn thách đấu bạn với mức cược " + goldThachDau);
    ...
    public void sendInvitePVP(Player pl, Player plMap) {
        ...
        try {
            ...
            msg.writer().writeUTF(pl.name + " (sức mạnh " + Util.powerToString(pl.nPoint.power) + ") muốn luyện tập với bạn");
            plMap.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
        }
