...
public class Clan {
    ...
    public int bdkb_level;
    public long bdkb_time;
    ...
    public void update() {
        ...
        try (Connection con = AlyraManager.getConnection();) {
            ps = con.prepareStatement("update clan set slogan = ?, img_id = ?, power_point = ?, max_member = ?, clan_point = ?, "
                    + "level = ?, members = ?, name_2 = ?, tops = ?, bdkb_level = ?, bdkb_time = ? where id = ? limit 1"); // Thêm bdkb_level, bdkb_time
            ...
            ps.setLong(11, this.bdkb_time);

            ps.setInt(12, this.id); // Đổi index id thành 12
