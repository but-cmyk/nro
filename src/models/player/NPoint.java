...
public class NPoint {
    ...
    public boolean buffDefenseSatellite, buffExpSatellite;
...
//    public Item trainArmor;
    ...
    private void setPointWhenWearClothes() {
        resetPoint();
        //nội tại vàng
        this.intrinsic = this.player.playerIntrinsic.intrinsic;
        final int ID_NOI_TAI_TANG_VANG = 23;
...
//    }
    ...
    public long calSucManhTiemNang(long tiemNang) {
        if (power < getPowerLimit()) {
            ...
            long tn1 = tiemNang;
            if (this.player.itemTime != null && this.player.itemTime.isTnDeTu) {
                tiemNang += tn1 * 2;
            }
...
//            if (this.player.getSession() != null && this.player.getSession().vip > 0 && System.currentTimeMillis() < this.player.getSession().lastTimeReceivedVIP + (1000 * 60 * 60 * 24 * 7) || this.player.isPet
//                    && ((Pet) this.player).master.getSession() != null && ((Pet) this.player).master.getSession().vip > 0 && System.currentTimeMillis() < ((Pet) this.player).master.getSession().lastTimeReceivedVIP + (1000 * 60 * 60 * 24 * 7)) {
//                tiemNang += tn * 3;
//            }
            if (this.player.itemTime != null && this.player.itemTime.isUseDK) {
                tiemNang += tn * 2;
            }
            if (this.player.itemTime != null && this.player.itemTime.isBuaTNSM) {
                tiemNang += tn * 2;
            }
...
//            }
    ...
    public void powerUp(long power) {
        this.power += power;
        long limit = getPowerLimit();
        // Nếu sức mạnh sau khi cộng lớn hơn giới hạn thì gán bằng giới hạn
        if (this.power > limit) {
            this.power = limit;
        }
    ...
    public boolean canOpenNextPowerLimit() {
        // Điều kiện 1: Mốc hiện tại phải nhỏ hơn mốc tối đa.
        // Điều kiện 2: Sức mạnh hiện tại phải lớn hơn hoặc bằng giới hạn của mốc hiện tại.
        return this.limitPower < NPoint.MAX_LIMIT && this.power >= this.getPowerLimit();
    }
